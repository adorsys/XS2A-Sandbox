...
export class PeriodicPaymentsComponent implements OnInit {
  ...
  openCancelPaymentModal(content, contentSelect, contentTan, contentDeleted, payment: PaymentTO) {
    this.targetedPayment = payment;
    this.modalService.open(content).result.then(
      () => {
        this.initCancel(contentSelect, contentTan, contentDeleted, this.targetedPayment.paymentId);
      }, () => {
      }
    );
  }
  ...
  initCancel(contentSelect, contentTan, contentDeleted, paymentId: string) {
    this.onlineBankingService.startCancellation(paymentId)
      .subscribe((response) => {
        ...
        if (response.scaMethods.length == 0) {
          ...
          throw new HttpErrorResponse({status:401,statusText:'No Sca methods available! Cancellation not possible!'});
        }
  ...
  startSca(contentTan, contentDeleted) {
    return this.onlineBankingService.startScaNselectMethod(this.cancellationResponse.paymentId, this.cancellationResponse.chosenScaMethod.id, this.makeString())
      .subscribe((response) => {
        this.cancellationResponse = response;
        this.authService.setAuthToken(response.bearerToken.access_token);
  ...
  validateNcancel(contentDeleted) {
    return this.onlineBankingService.validateTanAndExecuteCancellation(this.cancellationResponse.paymentId, this.cancellationResponse.authConfirmationCode, this.cancellationResponse.authorisationId)
      .subscribe((response) => {
          this.cancellationResponse = response;
