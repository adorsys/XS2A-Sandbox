...
describe('TanConfirmationComponent', () => {
  ...
  beforeEach(async(() => {
    TestBed.configureTestingModule({
      ...
      declarations: [TanConfirmationComponent, AccountDetailsComponent],
        providers: [CustomizeService, ShareDataService, AisService,
  ...
  it('should create', () => {
    ...
    it('should call the on submit', () => {
        ...
        let pisAuthSpy = spyOn(aisService, 'authrizedConsent').and.returnValue(of(mockResponse));
        let navigateSpy = spyOn(router, 'navigate').and.returnValue(of(undefined).toPromise());
        component.onSubmit();
        expect(navigateSpy).toHaveBeenCalledWith([`${RoutingPath.ACCOUNT_INFORMATION}/${RoutingPath.RESULT}`],
    ...
    it('should call the on submit and return to result page when you set a wrong TAN', () => {
        ...
        let pisAuthSpy = spyOn(aisService, 'authrizedConsent').and.returnValue(throwError(mockResponse));
        let navigateSpy = spyOn(router, 'navigate').and.returnValue(of(undefined).toPromise());
        component.onSubmit();
        expect(navigateSpy).toHaveBeenCalledWith([`${RoutingPath.ACCOUNT_INFORMATION}/${RoutingPath.RESULT}`],
    ...
    it('should cancel and redirect to result page', () => {
        ...
        component.authResponse = mockResponse;
        let revokeSpy = spyOn(aisService, 'revokeConsent').and.returnValue(of(mockResponse));
        let navigateSpy = spyOn(router, 'navigate').and.returnValue(of(undefined).toPromise());;
        component.onCancel();
        expect(navigateSpy).toHaveBeenCalledWith([`${RoutingPath.ACCOUNT_INFORMATION}/${RoutingPath.RESULT}`],
            {queryParams: { encryptedConsentId:'owirhJHGVSgueif98200293uwpgofowbOUIGb39845zt0',
