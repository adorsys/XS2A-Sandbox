...
public class ConsentHelper {
    ...
    private ResponseEntity<ConsentsResponse201> createConsent(Consents consents) {
        ...
        Assert.assertNotNull(getLink(consent201.getLinks(), "updatePsuAuthentication"));
...
//		access.setAllPsd2(AllPsd2Enum.ALLACCOUNTS);
    ...
    public ResponseEntity<UpdatePsuAuthenticationResponse> login(
        ...
        String startAuthorisationWithPsuAuthentication = getLink(consentsResponse201.getLinks(), "updatePsuAuthentication");
    ...
    public ResponseEntity<UpdatePsuAuthenticationResponse> authCode(UpdatePsuAuthenticationResponse authResponse) {
        AuthUrl authUrl = AuthUrl.parse(getLink(authResponse.getLinks(), "authoriseTransaction"));
    ...
    public ResponseEntity<UpdatePsuAuthenticationResponse> choseScaMethod(
        ...
        AuthUrl authUrl = AuthUrl.parse(getLink(authResponse.getLinks(), "selectAuthenticationMethod"));
        String authorisationId = authUrl.getAuthorizationId();
        String encryptedConsentId = authUrl.getEncryptedConsentId();
    ...
    public Map<String, Map<String, TransactionList>> loadTransactions(UpdatePsuAuthenticationResponse authCodeResponse, Boolean withBalance) {
        UUID xRequestID = UUID.randomUUID();
        ...
        accounts.forEach(a -> {
            Map<String, TransactionList> loadTransactions = loadTransactions(a, authUrl, withBalance);
    ...
    private Map<String, TransactionList> loadTransactions(AccountDetails a, AuthUrl authUrl, Boolean withBalance) {
        UUID xRequestID = UUID.randomUUID();
